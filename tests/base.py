...
class LiveTestBase(TestBase):
    ...
    def setUp(self, auth_mode='ApiUser'):
        ...
        sg_info = self.sg.info()
        self.sg_version = tuple(sg_info['version'][:3])
        ...
        if self.sg.server_caps.version and \
            ...
            self.server_address = re.sub('^0.0.0.0|127.0.0.1', 'localhost',
                                         self.sg.server_caps.host)
        else:
    ...
    def _setup_db(self, config):
        ...
        self.local_storage = _find_or_create_entity(self.sg, 'LocalStorage',
                                                    data, keys)
