...
class TestShotgunApi(base.LiveTestBase):
    ...
    def test_upload_download(self):
        ...
        ticket = self.sg.find_one('Ticket', [['id', 'is', self.ticket['id']]],
                                  ['attachments'])
        attach_file = self.sg.download_attachment(ticket['attachments'][-1])
        self.assertTrue(attach_file is not None)
        self.assertEqual(size, len(attach_file))
        self.assertEqual(orig_file, attach_file)

        # test download with attachment hash (write to disk)
        result = self.sg.download_attachment(ticket['attachments'][-1],
                                             file_path=file_path)
        self.assertEqual(result, file_path)
        fp = open(file_path, 'rb')
        attach_file = fp.read()
    ...
    def test_upload_thumbnail_in_create(self):
        ...
        data = {
            'filmstrip_image': path, 'code': 'Test Version',
            'project': self.project
            }
        new_version = self.sg.create("Version", data,
                                     return_fields=['filmstrip_image'])
        self.assertTrue(new_version is not None)
        self.assertTrue(isinstance(new_version, dict))
        self.assertTrue(isinstance(new_version.get('id'), int))
        self.assertEqual(new_version.get('type'), 'Version')
        ...
        h = Http(".cache")
        filmstrip_thumb_resp, content = h.request(
            new_version.get('filmstrip_image'), "GET")
    ...
    def test_upload_thumbnail_for_version(self):
        ...
        h = Http(".cache")
        thumb_resp, content = h.request(version_with_thumbnail.get('image'),
                                        "GET")
        self.assertEqual(thumb_resp['status'], '200')
        self.assertEqual(thumb_resp['content-type'], 'image/jpeg')
        ...
        expected_clear_thumbnail = {
            'id': self.version['id'], 'image': None, 'type': 'Version'
            }
    ...
    def test_upload_thumbnail_with_upload_function(self):
        ...
        path = os.path.abspath(os.path.expanduser(
            os.path.join(os.path.dirname(__file__), "sg_logo.jpg")))
        ...
        f_thumb_id = self.sg.upload("Task", self.task['id'], path,
                                    'filmstrip_image')
    ...
    def test_linked_thumbnail_url(self):
        ...
        if self.sg.server_caps.version and self.sg.server_caps.version >= (
        3, 3, 0):

            self.assertEqual(response_version_with_project[0].get('type'),
                             'Version')
            self.assertEqual(response_version_with_project[0].get('id'),
                             self.version['id'])
            self.assertEqual(response_version_with_project[0].get('code'),
                             'Sg unittest version')

            h = Http(".cache")
            thumb_resp, content = h.request(
                response_version_with_project[0].get('project.Project.image'),
                "GET")
            self.assertEqual(thumb_resp['status'], '200')
            self.assertEqual(thumb_resp['content-type'], 'image/jpeg')
            ...
            self.assertEqual(expected_version_with_project,
                             response_version_with_project)
    ...
    def test_ensure_ascii(self):
        ...
        result = sg_ascii.find_one('Note', [['id', 'is', self.note['id']]],
                                   fields=['content'])
    ...
    def test_ensure_unicode(self):
        ...
        sg_unicode = shotgun_api3.Shotgun(self.config.server_url,
                                          ...
                                          self.config.api_key,
                                          ensure_ascii=False)
        result = sg_unicode.find_one('Note', [['id', 'is', self.note['id']]],
                                     fields=['content'])
    ...
    def test_work_schedule(self):
        ...
        work_schedule = self.sg.work_schedule_read(start_date, end_date,
                                                   project, user)
        ...
        resp = self.sg.work_schedule_update('2012-01-02', False,
                                            'Studio Holiday')
        expected = {
            'date': '2012-01-02',
        ...
        work_schedule['2012-01-02'] = {
            "reason": "STUDIO_EXCEPTION", "working": False,
            "description": "Studio Holiday"
            }
        self.assertEqual(work_schedule, resp)

        resp = self.sg.work_schedule_update('2012-01-03', False,
                                            'Project Holiday', project)
        expected = {
            'date': '2012-01-03',
        ...
        work_schedule['2012-01-03'] = {
            "reason": "PROJECT_EXCEPTION", "working": False,
            "description": "Project Holiday"
            }
        self.assertEqual(work_schedule, resp)
        ...
        self.assertRaises(shotgun_api3.ShotgunError,
                          self.sg.work_schedule_update, jan4, False,
                          'Artist Holiday', user=user)

        resp = self.sg.work_schedule_update("2012-01-04", False,
                                            'Artist Holiday', user=user)
        expected = {
            'date': '2012-01-04',
        ...
        work_schedule['2012-01-04'] = {
            "reason": "USER_EXCEPTION", "working": False,
            "description": "Artist Holiday"
            }
        self.assertEqual(work_schedule, resp)
        ...
        if multi_entity_update_mode:
            self.sg.update(entity, entity_id, {field_name: new_value},
                           multi_entity_update_modes={
                               field_name: multi_entity_update_mode
                               })
        else:
            self.sg.update(entity, entity_id, {field_name: new_value})
...
class TestUtc(base.LiveTestBase):
    ...
    def setUp(self):
        ...
        local = shotgun_api3.shotgun.SG_TIMEZONE.local
        self.datetime_local = datetime.datetime(2008, 10, 13, 23, 10,
                                                tzinfo=local)
    ...
    def _assert_expected(self, sg, date_time, expected):
        ...
        result = sg.find_one(entity_name, [['id', 'is', entity_id]],
                             [field_name])
...
class TestFind(base.LiveTestBase):
    def setUp(self):
        ...
        self.shot = self.sg.find_one('Shot', [['id', 'is', self.shot['id']]],
                                     fields)
        # We will need the uuid field for our LocalStorage
        fields = list(self.local_storage.keys())[:]
        fields.append('uuid')
        self.local_storage = self.sg.find_one('LocalStorage', [
            ['id', 'is', self.local_storage['id']]], fields)
    ...
    def test_not_in_relation_text(self):
        ...
        """
        filters = [
            ['name', 'not_in', [self.project['name'], 'fake project name']]]
        result = self._id_in_result('Project', filters, self.project['id'])
        self.assertFalse(result)
    ...
    def test_in_relation_comma_duration(self):
        ...
        self.task = self.sg.find_one('Task', [['id', 'is', self.task['id']]],
                                     new_task_keys)
    ...
    def test_in_relation_comma_entity_type(self):
        ...
        """
        filters = [
            ['entity_type', 'in', self.step['entity_type'], 'something else']]
    ...
    def test_not_in_relation_entity_type(self):
        ...
        """
        filters = [['entity_type', 'not_in',
                    [self.step['entity_type'], 'something else']]]
    ...
    def test_in_relation_comma_float(self):
        ...
        """
        filters = [['sg_frames_aspect_ratio', 'in',
                    self.version['sg_frames_aspect_ratio'], 44.0],
                   ['project', 'is', self.project]]
    ...
    def test_not_in_relation_float(self):
        ...
        """
        filters = [['sg_frames_aspect_ratio', 'not_in',
    ...
    def test_in_relation_comma_uuid(self):
        ...
        result = self._id_in_result('LocalStorage', filters,
                                    self.local_storage['id'])
        self.assertTrue(result)
    ...
    def test_unsupported_filters(self):
        ...
        self.assertRaises(shotgun_api3.Fault, self.sg.find_one, 'HumanUser',
                          [['password_proxy', 'is_not', [None]]])
        self.assertRaises(shotgun_api3.Fault, self.sg.find_one, 'EventLogEntry',
                          [['meta', 'is_not', [None]]])
        self.assertRaises(shotgun_api3.Fault, self.sg.find_one, 'Revision',
    ...
    def test_zero_is_not_none(self):
        ...
        if num_field not in self.sg.schema_field_read('Asset').keys():
            self.sg.schema_field_create('Asset', 'number',
                                        num_field.replace('sg_', '').replace(
                                            '_', ' '))
        ...
        result = self.sg.find('Asset', [['id', 'is', self.asset['id']],
                                        [num_field, 'is_not', None]],
                              [num_field])
        ...
        result = self.sg.find_one('Asset', [['id', 'is', self.asset['id']],
                                            [num_field, 'is_not', None]],
                                  [num_field])
...
class TestFollow(base.LiveTestBase):
    def setUp(self):
        ...
        self.sg.update('HumanUser', self.human_user['id'],
...
class TestErrors(base.TestBase):
    def test_bad_auth(self):
        ...
        self.assertRaises(ValueError, shotgun_api3.Shotgun, server_url, None,
                          api_key)
        self.assertRaises(ValueError, shotgun_api3.Shotgun, server_url,
                          script_name, None)
        self.assertRaises(ValueError, shotgun_api3.Shotgun, server_url,
                          script_name, api_key, login=login, password=password)
        self.assertRaises(ValueError, shotgun_api3.Shotgun, server_url,
                          login=login)
        self.assertRaises(ValueError, shotgun_api3.Shotgun, server_url,
                          password=password)
        self.assertRaises(ValueError, shotgun_api3.Shotgun, server_url,
                          script_name, login=login, password=password)
        self.assertRaises(ValueError, shotgun_api3.Shotgun, server_url,
                          login=login, auth_token=auth_token)
        self.assertRaises(ValueError, shotgun_api3.Shotgun, server_url,
                          password=password, auth_token=auth_token)
        self.assertRaises(ValueError, shotgun_api3.Shotgun, server_url,
                          script_name, login=login,
                          password=password, auth_token=auth_token)
        self.assertRaises(ValueError, shotgun_api3.Shotgun, server_url,
                          api_key=api_key, login=login,
                          password=password, auth_token=auth_token)
        ...
        sg = shotgun_api3.Shotgun(server_url, login=login,
    ...
    def test_sha2_error(self, mock_request):
        ...
        original_env_val = os.environ.pop(
            "SHOTGUN_FORCE_CERTIFICATE_VALIDATION", None)
        ...
        if original_env_val is not None:
            os.environ[
                "SHOTGUN_FORCE_CERTIFICATE_VALIDATION"] = original_env_val
...
class TestSessionTokenAuth(base.SessionTokenAuthLiveTestBase):
    ...
    def test_humanuser_upload_thumbnail_for_version(self):
        ...
        if self.sg.server_caps.version >= (5, 4, 1):
            ...
            self.assertEqual(version_with_thumbnail.get('type'), 'Version')
            self.assertEqual(version_with_thumbnail.get('id'),
                             self.version['id'])
...
class TestProjectLastAccessedByCurrentUser(base.LiveTestBase):
    ...
    def test_logged_in_user(self):
        ...
        initial = sg.find_one('Project', [['id', 'is', self.project['id']]],
                              ['last_accessed_by_current_user'])

        sg.update_project_last_accessed(self.project)

        current = sg.find_one('Project', [['id', 'is', self.project['id']]],
                              ['last_accessed_by_current_user'])
        self.assertNotEqual(initial, current)
        # it's possible initial is None
        if initial:
            assert (initial['last_accessed_by_current_user'] < current[
                'last_accessed_by_current_user'])
...
class TestActivityStream(base.LiveTestBase):
    ...
    def setUp(self):
        ...
        self._shot = self.sg.create("Shot", {
            "code": "%s activity stream test" % self._prefix,
            "project": self.project
            })

        self._note = self.sg.create("Note", {
            "content": "Test!",
            "project": self.project,
...
class TestNoteThreadRead(base.LiveTestBase):
    ...
    def setUp(self):
        ...
        this_dir, _ = os.path.split(__file__)
        self._thumbnail_path = os.path.abspath(
            os.path.join(this_dir, "sg_logo.jpg"))

    def _check_note(self, data, note_id, additional_fields):

        # check the expected fields
        expected_fields = set(["content", "created_at", "created_by", "id",
    ...
    def test_simple(self):
        ...
        note = self.sg.create("Note",
                              {"content": "Test!", "project": self.project})
        ...
        # now add a reply
        reply = self.sg.create("Reply",
                               {"content": "Reply Content", "entity": note})
        ...
        attachment_id = self.sg.upload(note["type"], note["id"],
                                       self._thumbnail_path)
    ...
    def test_complex(self):
        ...
        self._check_attachment(result[2], attachment_id,
                               additional_fields["Attachment"])
...
class TestReadAdditionalFilterPresets(base.LiveTestBase):
    ...
    def test_simple_case(self):
        if self.sg_version < (7, 0, 0):
            warnings.warn(
                "Test bypassed because SG server used does not support this feature.",
                FutureWarning)
            return
